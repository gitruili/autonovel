...
Usage: `genre = load_genre_for_project()` reads `story/project.json` → genre field → loads YAML → deep-merges with `_base.yaml`. Falls back to 总裁豪门 if genre is empty or not found.
