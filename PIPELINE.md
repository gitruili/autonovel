...
```
输入:  seed.txt（用户提供或通过 seed.py / seed_lf.py 生成）
输出:  创建分支，配置 .env
...
3. 生成或提供 seed.txt：
   - 方式 A：`autonovel_cli.py generate seed` 生成多个概念，手动选择保存到 seed.txt
   - 方式 B：`autonovel_cli.py generate seed --long-form --count 5 --market-research reports/XXX.md` 生成长篇脑洞（自动分批）
   - 方式 C：`autonovel_cli.py generate seed --input-file concept.txt` 评价+优化用户已有概念
