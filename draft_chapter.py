...
=== 写作指令 ===
1. 章节开头必须输出标题行：`# 第{chapter_num}章 标题`（标题取自大纲中指定的章节标题）。
2. 撰写完整的章节。目标字数约 3,500-4,500 字。不要截断或总结。
3. 第三人称限制视角，过去时态，紧贴大纲中指定的视角人物。
4. 按顺序完成大纲中所有关键场景。
5. 植入大纲中"伏笔植入"项下列出的所有元素。
6. 展示感官细节：参考世界设定集中"感官特征"部分，将触觉、嗅觉、听觉融入场景。
7. 经济数据必须与世界设定集中的物价表一致。提到金额时必须具体。
8. 对话遵循角色信息中定义的说话模式和身份特征。
9. 不要使用语气定义中禁用的词汇和句式。
