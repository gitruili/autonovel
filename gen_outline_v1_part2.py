#!/usr/bin/env python3
"""
gen_outline_v1_part2.py -- 长篇女频种田网文：第一卷大纲续写脚本。
当 gen_outline_v1.py 因输出长度限制未完成全部章节时，本脚本续写剩余章节并追加到 outline.md。
"""
import os
import sys
...
prompt = f"""这是一部百万字长篇女频种田网文第一卷大纲的前半部分。
大纲在某一章中途断开了。请从断开的地方继续，完成第一卷剩余章节（目标共20章），
然后补全以下台账（如果前半部分未包含）：
...
- 伏笔的回收要与前面的植入对应
- 打脸/爽点必须有前面的铺垫
- 第一卷最后1-2章必须留下悬念，驱动读者追第二卷
- 如果第一卷已经完成（已有20章），则只补全台账
"""

print("正在续写第一卷大纲...", file=sys.stderr)
