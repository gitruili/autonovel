...
# Load outline.md (generated by gen_master_outline.py in step 2)
...
这份文档分三部分：Part 0 是世界观速查表（结构化核心信息），Part A 是核心设定（卷1-3 立即需要的），Part B 是扩展路线图（卷4至大结局，一次性完整输出，不再分期补全）。
...
根据以上总纲中定义的25卷剧情走向、舞台切换节奏和扩展路径，在 Part B 中完整规划从卷4到大结局的全部地理扩展锚点、势力/层级扩展和世界观伏笔。每条锚点必须与总纲中对应卷号的具体剧情事件挂钩。
...
# Part B: 扩展路线图（卷4至大结局，根据总纲一次性完整输出）

这是百万字长篇独有的部分。总纲已确定了25卷的舞台切换和剧情走向，
请根据总纲中每一卷的事件和舞台变化，一次性完整输出所有扩展锚点。
每一个扩展锚点都要具体到可以直接用于写作的程度。

请按以下结构组织（确保覆盖从卷4到第25卷的全部扩展）：
